...
class MmsFileProvider : ContentProvider() {
    ...
    /**
     * Resolves [uri] to a file strictly inside cacheDir/mms_in or cacheDir/mms_out â€” this
     ...
     * â€” it still starts with "/mms_out/" but resolves outside that directory once "../" segments
     * are followed, letting a malicious caller read/write/delete files elsewhere. Using only
     * File(path).name â€” the final path segment â€” collapses any such traversal down to a bare
