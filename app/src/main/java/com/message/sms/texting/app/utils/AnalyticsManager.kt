...
object AnalyticsManager {
    ...
    /** Sets a Firebase user property (segment users by it in the console) â€” e.g. app_language,
     * is_default_sms_app. Persists across sessions until changed again. */
    fun setUserProperty(name: String, value: String?) {
        if (!enabled) return
        firebaseAnalytics?.setUserProperty(name, value)
    }

    /** Logs Firebase's standard screen_view event â€” call on every screen/destination change. */
