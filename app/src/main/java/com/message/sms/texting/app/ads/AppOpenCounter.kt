 ...
 * not backgroundâ†’foreground within the same session) and decides which ad â€” if any â€” Splash
 * should show that time. The true "very first ever open" (fresh install, finishing onboarding)
 * never reaches this counter at all â€” completing setup navigates straight to Dashboard without
 * passing back through Splash â€” so this counter's count=1 already represents the user's first
...
object AppOpenCounter {
    ...
    /** Reads the current stored count without incrementing it â€” for callers (e.g. Home, deciding
