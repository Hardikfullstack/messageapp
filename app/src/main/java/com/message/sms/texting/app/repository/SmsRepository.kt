...
class SmsRepository(private val context: Context) {
    ...
    suspend fun blockContact(address: String, contactName: String?) = withContext(Dispatchers.IO) {
        blockedContactDao.insert(com.message.sms.texting.app.model.BlockedContact(address, contactName))
        smsDao.updateIsBlockedStatus(address, true)
        AnalyticsManager.logEventWithAction("chat_blocked", "Chat", "block")
    }

    suspend fun blockContacts(contacts: List<com.message.sms.texting.app.model.BlockedContact>) = withContext(Dispatchers.IO) {
    ...
    suspend fun sendSms(address: String, body: String, threadId: Long, contactName: String?, delayMillis: Long = 0) = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val deliveryIntent = if (com.message.sms.texting.app.ui.theme.DeliveryConfirmationState.enabled.value) {
                val intent = android.content.Intent(context, com.message.sms.texting.app.receiver.SmsDeliveredReceiver::class.java).apply {
    ...
    private fun saveMmsImageBitmap(bitmap: android.graphics.Bitmap): java.io.File {
        ...
        val mode = com.message.sms.texting.app.ui.theme.MmsCompressionState.mode.value
        val targetBytes = com.message.sms.texting.app.ui.theme.MmsCompressionState.targetBytesFor(mode)
        ...
        when {
            ...
            targetBytes == null -> {
                // "automatic" â€” sensible default quality, no strict size target.
                java.io.FileOutputStream(file).use { out ->
    ...
    suspend fun sendMms(
        ...
        val success = try {
            ...
            com.message.sms.texting.app.mms.MmsSender.send(context, address, body.ifBlank { null }, imageBytes, "image/jpeg")
        } catch (e: Exception) {
            e.printStackTrace()
            false
        }
    ...
    suspend fun retryMms(id: Long) = withContext(Dispatchers.IO) {
        ...
        val success = try {
            com.message.sms.texting.app.mms.MmsSender.send(context, msg.address, msg.body.ifBlank { null }, imageFile.readBytes(), "image/jpeg")
        } catch (e: Exception) {
            e.printStackTrace()
            false
        }
    ...
    suspend fun deleteMessagesByAddress(address: String) = withContext(Dispatchers.IO) {
        ...
        if (ids.isNotEmpty()) {
            ...
            val deletedMsgs = ids.map { com.message.sms.texting.app.model.DeletedMessage(it) }
    ...
    suspend fun deleteThreads(threadIds: List<Long>) = withContext(Dispatchers.IO) {
        ...
        smsDao.insertDeletedMessages(messageIds.map { com.message.sms.texting.app.model.DeletedMessage(it) })
    }

    /** Normalizes [address] to bare digits (+ leading "+") for contact matching â€” returns null if
    ...
    // Scheduled Messages
    suspend fun insertScheduledMessage(message: com.message.sms.texting.app.model.ScheduledMessage): Long {
        return scheduledMessageDao.insert(message)
    }

    fun getAllScheduledMessages(): Flow<List<com.message.sms.texting.app.model.ScheduledMessage>> {
        return scheduledMessageDao.getAllScheduledMessages()
    }

    suspend fun updateScheduledMessage(message: com.message.sms.texting.app.model.ScheduledMessage) {
        scheduledMessageDao.update(message)
    }

    suspend fun getScheduledMessageById(id: Long): com.message.sms.texting.app.model.ScheduledMessage? {
    ...
    // Reminders
    suspend fun insertReminder(reminder: com.message.sms.texting.app.model.Reminder): Long {
        return reminderDao.insert(reminder)
    }

    suspend fun getRemindersByAddress(address: String): List<com.message.sms.texting.app.model.Reminder> {
        return reminderDao.getAllByAddress(address)
    }

    fun getRemindersByAddressFlow(address: String): Flow<List<com.message.sms.texting.app.model.Reminder>> {
        return reminderDao.getAllByAddressFlow(address)
    }

    suspend fun getReminderById(id: Long): com.message.sms.texting.app.model.Reminder? {
    ...
    // Drafts
    suspend fun getDraft(address: String): com.message.sms.texting.app.model.Draft? = withContext(Dispatchers.IO) {
    ...
    suspend fun saveDraft(address: String, threadId: Long, body: String) = withContext(Dispatchers.IO) {
        if (body.isBlank()) {
            ...
            draftDao.upsert(com.message.sms.texting.app.model.Draft(address, threadId, body, System.currentTimeMillis()))
        }
    }
    ...
    fun getAllDraftsFlow(): Flow<List<com.message.sms.texting.app.model.Draft>> {
        return draftDao.getAllDrafts()
    }

    // Groups (broadcast lists â€” sends land as individual SMS in each member's own thread)
    suspend fun createGroup(
        name: String,
        avatarUri: String?,
        members: List<com.message.sms.texting.app.model.GroupMember>
    ): Long = withContext(Dispatchers.IO) {
        groupDao.insert(
            com.message.sms.texting.app.model.Group(
                name = name,
                avatarUri = avatarUri,
                members = members,
    ...
    suspend fun getGroupById(id: Long): com.message.sms.texting.app.model.Group? = withContext(Dispatchers.IO) {
        groupDao.getGroupById(id)
    }

    fun getGroupByIdFlow(id: Long): Flow<com.message.sms.texting.app.model.Group?> {
        return groupDao.getGroupByIdFlow(id)
    }

    fun getAllGroupsFlow(): Flow<List<com.message.sms.texting.app.model.Group>> {
        return groupDao.getAllGroups()
    }

    fun getAllGroupsWithLastMessageFlow(): Flow<List<com.message.sms.texting.app.model.GroupWithLastMessage>> {
        return groupDao.getAllGroupsWithLastMessage()
    }

    fun searchGroupsByName(query: String): Flow<List<com.message.sms.texting.app.model.Group>> {
        return groupDao.searchGroupsByName(query)
    }

    fun searchGroupMessageContent(query: String): Flow<List<com.message.sms.texting.app.model.GroupSearchResult>> {
        return groupDao.searchGroupMessageContent(query)
    }

    suspend fun updateGroup(group: com.message.sms.texting.app.model.Group) = withContext(Dispatchers.IO) {
    ...
    fun getGroupMessagesFlow(groupId: Long): Flow<List<com.message.sms.texting.app.model.GroupMessage>> {
        return groupMessageDao.getMessagesForGroup(groupId)
    }

    suspend fun sendToGroup(group: com.message.sms.texting.app.model.Group, body: String, delayMillis: Long = 0) = withContext(Dispatchers.IO) {
        if (delayMillis > 0) {
            val pendingId = groupMessageDao.insert(
                com.message.sms.texting.app.model.GroupMessage(
                    groupId = group.id,
                    body = body,
                    date = System.currentTimeMillis(),
                    sendStatus = SmsMessage.STATUS_DELAYED,
