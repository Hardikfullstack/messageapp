...
/** Numbers CallLog uses for withheld/unknown numbers â€” nothing we can message/call back, so skip these. */
...
class AfterCallReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        // Only act on a transition INTO idle from an active call â€” not app startup or repeats.
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    val intent = Intent(appContext, com.message.sms.texting.app.ui.screens.AfterCallActivity::class.java).apply {
                    ...
                    // gets silently swallowed (OnePlus/OxygenOS in particular â€” see
                    ...
                    // broadcast open â€” on devices where the direct launch already works, this is a
                    // no-op, since isVisible flips true almost immediately.
                    CoroutineScope(Dispatchers.Main).launch {
                        delay(1000)
                        if (!com.message.sms.texting.app.ui.screens.AfterCallActivity.isVisible) {
                            com.message.sms.texting.app.NotificationHelper.showAfterCallFullScreenNotification(
                                appContext, intent
                            )
                        }
                    }
     ...
     * adding the row's own duration to estimate when it actually ended â€” a long answered call
    ...
    private fun checkCallLogRow(context: Context, callEndTimeMs: Long): CallLogMatch? {
        // No SQL-level LIMIT here â€” some OEM CallLogProvider implementations (e.g. MIUI's) reject
    ...
    /**
     * Waits for the just-ended call's CallLog row instead of polling on a fixed interval â€”
