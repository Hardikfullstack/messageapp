...
import com.message.sms.texting.app.data.network.ApiClient
 ...
 * available instantly on startup â€” before the network round trip completes â€” instead of null.
 */
class AppConfigViewModel(application: Application) : AndroidViewModel(application) {
    ...
    companion object {
         ...
         * ViewModel instance (e.g. a BroadcastReceiver) â€” used to kick off a native-ad preload
         * as early as possible after a call ends, before AfterCallScreen even composes. */
        fun readCachedResult(context: Context): com.message.sms.texting.app.data.model.AppResult? {
    ...
    private fun observeNetwork() {
        viewModelScope.launch {
            // NetworkMonitor emits the current status immediately on subscription â€” init{} already
            // did the initial fetchAppData(), so skip that first emission and only refetch on an
            // actual offlineâ†’online transition (real reconnect), not the startup snapshot.
    ...
    private fun fetchAppData() {
        viewModelScope.launch {
            try {
                ...
                if (response.status == 200) {
                    ...
                    response.result?.let { result ->
                        ...
                        try {
                            ...
                            // Cache-write failure â€” in-memory state above is already updated.
                        }
                ...
                // Offline or server error â€” cached/default state (already loaded) stays as-is.
            }
        }
    }
}
