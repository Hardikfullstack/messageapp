...
import com.messages.sms.texting.app.utils.SetupState

/**
 * Decides whether a given back-navigation (pop) anywhere in the app should trigger an
 * interstitial — every 4th one does, not every single back (spammy/policy-risky). The counter
 * resets after each trigger, so the pattern repeats: 3 backs with no ad, 4th shows one.
 *
 * Deliberately NOT triggered on the back-press that exits the app entirely (e.g. from Home) —
 ...
 * ready — improves fill rate without needing the caller to know about the fallback. Only the
 * primary is preloaded eagerly (kept warm the whole session); the fallback loads reactively,
 * only once the primary has actually failed to be ready at a trigger — no point holding two ad
 * objects warm at all times for a trigger that only ever shows one of them.
 *
 * This object only *decides*; it doesn't call [InterstitialAdManager.show] itself — see the
...
object GlobalBackAdManager {
     ...
     * available, or null otherwise — showing it is the caller's responsibility.
     */
    fun resolveAdUnitIdForBackPress(context: Context): String? {
        ...
        // Primary wasn't ready this time — start warming the fallback so it has a chance to be
