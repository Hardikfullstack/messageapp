 ...
 * is even required for the user's region (EEA/UK/applicable US states) — everywhere else (e.g.
 * India, where this app is mostly tested) this resolves near-instantly with no form ever shown.
 *
 * Once resolved here, the rest of the session's ad requests — all the scattered preload/show call
 * sites elsewhere in the app (Home, Chat, Schedule, After Call, back-nav, ...) — don't need their
...
object UmpConsentManager {
    ...
    suspend fun gatherConsent(activity: Activity) {
        ...
        // already recorded from a previous successful session — never blocks ads forever just
