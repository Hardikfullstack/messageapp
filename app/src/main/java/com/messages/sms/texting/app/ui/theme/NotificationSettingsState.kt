 ...
 * "wake_screen_$threadId" in the separate "contact_notification_prefs" file) still win when set —
...
object NotificationSettingsState {
    ...
    /** Controls the notification channel's importance — HIGH shows the heads-up popup banner, DEFAULT stays silent in the tray. */
    var quickReplyEnabled = mutableStateOf(true)

    /**
     * Controls NotificationCompat.setAutoCancel — ON clears the notification automatically when
    ...
    /** Reads straight from SharedPreferences — safe to call from NotificationHelper/receivers without composable state. */
