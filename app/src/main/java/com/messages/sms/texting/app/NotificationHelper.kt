...
import com.messages.sms.texting.app.receiver.NotificationActionReceiver
import com.messages.sms.texting.app.ui.theme.NotificationSettingsState
...
object NotificationHelper {
    ...
    /** One shared channel for all SMS notifications (not per-contact) — matches how most messaging
     * apps structure this, and lets Settings → Notification deep-link to a single, clean channel
     * screen instead of a list of many per-contact channels. */
    const val DEFAULT_CHANNEL_ID = "default_sms_channel"

    /** Creates the shared channel if it doesn't exist yet. Importance is only set at creation —
    ...
    /** Separate small channel for After Call "call back" reminders — distinct from SMS notifications
     ...
     * [showAfterCallFullScreenNotification]) — needs its own IMPORTANCE_HIGH channel, distinct
     ...
     * AfterCallReceiver) — some OEMs (OnePlus/OxygenOS in particular) silently swallow a plain
     ...
     * heads-up notification instead — a working fallback either way, instead of nothing at all.
     */
     ...
     * solid [backgroundColorRes] circle — a notification largeIcon otherwise renders that
    ...
    fun showSmsNotification(
        ...
        // OTP quick-copy — only when the full preview is showing (previewOption 0); with the
        ...
        if (previewOption == 0) {
            com.messages.sms.texting.app.utils.extractOtpCode(body)?.let { code ->
