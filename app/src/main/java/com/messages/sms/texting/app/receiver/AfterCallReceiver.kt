...
class AfterCallReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val cachedResult = com.messages.sms.texting.app.viewmodel.AppConfigViewModel.readCachedResult(appContext)
        if (cachedResult?.google_ads_on_off == "on" && cachedResult.native_7_on_off == "on") {
            cachedResult.native_7?.takeIf { it.isNotBlank() }?.let {
                com.messages.sms.texting.app.ads.NativeAdCache.preload(appContext, it)
            }
        }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    val intent = Intent(appContext, com.messages.sms.texting.app.ui.screens.AfterCallActivity::class.java).apply {
                    ...
                    CoroutineScope(Dispatchers.Main).launch {
                        delay(1000)
                        if (!com.messages.sms.texting.app.ui.screens.AfterCallActivity.isVisible) {
                            com.messages.sms.texting.app.NotificationHelper.showAfterCallFullScreenNotification(
                                appContext, intent
                            )
                        }
                    }
