...
import com.messages.sms.texting.app.MainActivity
import com.messages.sms.texting.app.ui.theme.AfterCallReturnState
import com.messages.sms.texting.app.ui.theme.MessagesTheme

// AppCompatActivity (not plain ComponentActivity) — needed so the app's Light/Dark/System theme
...
class AfterCallActivity : AppCompatActivity() {
    ...
    // Every close path (X/back, leave-hint, the "Message" tap flow) routes through finish() —
    ...
    override fun finish() {
        com.messages.sms.texting.app.ads.AppOpenBackgroundReturnTrigger.isAdPaused = true
        super.finish()
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Bottom nav bar stays hidden on this screen — a swipe from the edge can still reveal it
    ...
    // instance (FLAG_ACTIVITY_SINGLE_TOP) instead of creating a new one — without this override,
    ...
    private fun updateFromIntent(intent: Intent): Boolean {
        ...
        // This screen is now genuinely showing the data — the full-screen-intent fallback (if one
        // was posted because the earlier direct launch attempt seemed to have been blocked) is no
        // longer needed.
        com.messages.sms.texting.app.NotificationHelper.cancelAfterCallFullScreenNotification(this)
        return true
    }

    // Returning to this Activity (e.g. after the mail/calendar chooser closes) resets the system
    // bars — re-hide the nav bar each time this window regains focus so it doesn't creep back.
