...
class SmsRepository(private val context: Context) {
    ...
    suspend fun blockContact(address: String, contactName: String?) = withContext(Dispatchers.IO) {
        blockedContactDao.insert(com.messages.model.BlockedContact(address, contactName))
        smsDao.updateIsBlockedStatus(address, true)
        AnalyticsManager.logEventWithAction("chat_blocked", "Chat", "block")
    }

    suspend fun blockContacts(contacts: List<com.messages.model.BlockedContact>) = withContext(Dispatchers.IO) {
    ...
    // Set up ContentObserver to listen for new SMS
    ...
    suspend fun sendSms(address: String, body: String, threadId: Long, contactName: String?, delayMillis: Long = 0) = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val deliveryIntent = if (com.messages.ui.theme.DeliveryConfirmationState.enabled.value) {
                val intent = android.content.Intent(context, com.messages.receiver.SmsDeliveredReceiver::class.java).apply {
    ...
    private fun saveMmsImageBitmap(bitmap: android.graphics.Bitmap): java.io.File {
        ...
        val mode = com.messages.ui.theme.MmsCompressionState.mode.value
        val targetBytes = com.messages.ui.theme.MmsCompressionState.targetBytesFor(mode)
    ...
    suspend fun sendMms(
        ...
        val success = try {
            ...
            com.messages.mms.MmsSender.send(context, address, body.ifBlank { null }, imageBytes, "image/jpeg")
        } catch (e: Exception) {
            e.printStackTrace()
            false
        }
    ...
    suspend fun retryMms(id: Long) = withContext(Dispatchers.IO) {
        ...
        val success = try {
            com.messages.mms.MmsSender.send(context, msg.address, msg.body.ifBlank { null }, imageFile.readBytes(), "image/jpeg")
        } catch (e: Exception) {
            e.printStackTrace()
            false
        }
    ...
    fun searchMessageContent(query: String): Flow<List<com.messages.model.MessageSearchResult>> {
    ...
    suspend fun performSync() = withContext(Dispatchers.IO) {
        ...
        if (deletedFromSystem.isNotEmpty()) {
    ...
    suspend fun deleteMessagesByAddress(address: String) = withContext(Dispatchers.IO) {
        ...
        if (ids.isNotEmpty()) {
            ...
            val deletedMsgs = ids.map { com.messages.model.DeletedMessage(it) }
    ...
    suspend fun markThreadAsRead(threadId: Long) = withContext(Dispatchers.IO) {
        try {
            // Update Android Telephony Provider
    ...
    suspend fun deleteMessages(ids: List<Long>) = withContext(Dispatchers.IO) {
        ...
        smsDao.insertDeletedMessages(ids.map { com.messages.model.DeletedMessage(it) })
    }
    ...
    suspend fun deleteThreads(threadIds: List<Long>) = withContext(Dispatchers.IO) {
        ...
        smsDao.insertDeletedMessages(messageIds.map { com.messages.model.DeletedMessage(it) })
    }
        ...
        withContext(Dispatchers.IO) {
            ...
            cursor?.use {
                ...
                while (it.moveToNext()) {
                    ...
                    var contactName: String? = null
                    var photoUri: String? = null
                    val isKnown = if (normalizedAddress.isNotBlank()) {
                        var found = false
                        for ((number, info) in knownContacts) {
                            if (number.endsWith(normalizedAddress) || normalizedAddress.endsWith(number)) {
                                contactName = info.name
    ...
    // Scheduled Messages
    suspend fun insertScheduledMessage(message: com.messages.model.ScheduledMessage): Long {
        return scheduledMessageDao.insert(message)
    }

    fun getAllScheduledMessages(): Flow<List<com.messages.model.ScheduledMessage>> {
        return scheduledMessageDao.getAllScheduledMessages()
    }

    suspend fun updateScheduledMessage(message: com.messages.model.ScheduledMessage) {
        scheduledMessageDao.update(message)
    }

    suspend fun getScheduledMessageById(id: Long): com.messages.model.ScheduledMessage? {
    ...
    // Reminders
    suspend fun insertReminder(reminder: com.messages.model.Reminder): Long {
        return reminderDao.insert(reminder)
    }

    suspend fun getRemindersByAddress(address: String): List<com.messages.model.Reminder> {
        return reminderDao.getAllByAddress(address)
    }

    fun getRemindersByAddressFlow(address: String): Flow<List<com.messages.model.Reminder>> {
        return reminderDao.getAllByAddressFlow(address)
    }

    suspend fun getReminderById(id: Long): com.messages.model.Reminder? {
    ...
    // Drafts
    suspend fun getDraft(address: String): com.messages.model.Draft? = withContext(Dispatchers.IO) {
    ...
    suspend fun saveDraft(address: String, threadId: Long, body: String) = withContext(Dispatchers.IO) {
        if (body.isBlank()) {
            ...
            draftDao.upsert(com.messages.model.Draft(address, threadId, body, System.currentTimeMillis()))
        }
    }
    ...
    fun getAllDraftsFlow(): Flow<List<com.messages.model.Draft>> {
    ...
    suspend fun createGroup(
        name: String,
        avatarUri: String?,
        members: List<com.messages.model.GroupMember>
    ): Long = withContext(Dispatchers.IO) {
        groupDao.insert(
            com.messages.model.Group(
                name = name,
                avatarUri = avatarUri,
                members = members,
    ...
    suspend fun getGroupById(id: Long): com.messages.model.Group? = withContext(Dispatchers.IO) {
        groupDao.getGroupById(id)
    }

    fun getGroupByIdFlow(id: Long): Flow<com.messages.model.Group?> {
        return groupDao.getGroupByIdFlow(id)
    }

    fun getAllGroupsFlow(): Flow<List<com.messages.model.Group>> {
        return groupDao.getAllGroups()
    }

    fun getAllGroupsWithLastMessageFlow(): Flow<List<com.messages.model.GroupWithLastMessage>> {
        return groupDao.getAllGroupsWithLastMessage()
    }

    fun searchGroupsByName(query: String): Flow<List<com.messages.model.Group>> {
        return groupDao.searchGroupsByName(query)
    }

    fun searchGroupMessageContent(query: String): Flow<List<com.messages.model.GroupSearchResult>> {
        return groupDao.searchGroupMessageContent(query)
    }

    suspend fun updateGroup(group: com.messages.model.Group) = withContext(Dispatchers.IO) {
    ...
    fun getGroupMessagesFlow(groupId: Long): Flow<List<com.messages.model.GroupMessage>> {
        return groupMessageDao.getMessagesForGroup(groupId)
    }

    suspend fun sendToGroup(group: com.messages.model.Group, body: String, delayMillis: Long = 0) = withContext(Dispatchers.IO) {
        if (delayMillis > 0) {
            val pendingId = groupMessageDao.insert(
                com.messages.model.GroupMessage(
                    groupId = group.id,
                    body = body,
                    date = System.currentTimeMillis(),
                    sendStatus = SmsMessage.STATUS_DELAYED,
