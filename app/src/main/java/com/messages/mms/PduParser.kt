 ...
 * message, fetched separately via [MmsDownloader]). Mirrors [PduComposer]'s scope — just enough
...
object PduParser {
    ...
    private fun readTextStringBytes(stream: ByteArrayInputStream): ByteArray {
        var b = stream.read()
        if (b == 0x7F) { // Quote octet — value follows, not part of the text
            b = stream.read()
        }
    ...
    fun parseRetrieveConf(data: ByteArray): RetrievedMessage {
        ...
        while (stream.available() > 0) {
            ...
            if (fieldCode == FIELD_CONTENT_TYPE) {
                // Content-Type value, then the multipart body immediately follows — this is
